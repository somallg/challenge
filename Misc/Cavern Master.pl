...
$m->get('http://www.hacker.org/challenge/misc/d/cavern.php');
open(FILE, ">", "Cavern.log");
my @nbAttack;
...
while(1) {
	...
	if ($m->content =~ m/Pick up treasure/) {
		...
		if ($m->content =~ m/sword/) {
			...
			if ($newWeapon > $currentWeapon) {
				...
				$m->get('http://www.hacker.org/challenge/misc/d/cavern.php?tres=1');
			}

			$newDmg = 0 unless defined($newDmg);
			$currentDmg = 9 unless defined($currentDmg);
	...
	if (($m->content =~ m/Down/) && ($level >= ($dungeonLevel + 5))) {
		...
		#chomp($input);
		#if ($input eq "y") {
		$m->get('http://www.hacker.org/challenge/misc/d/cavern.php?m=d');
		#}
	}
